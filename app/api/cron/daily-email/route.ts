...
export async function GET(request: Request) {
  ...
  try {
          ...
          const { data: rankingData, error: rankingError } = await rankQuery;
            ...
            const rankingRaw = (rankingData ?? []) as RankingEntry[];
          ...
          const profiles = (profilesData ?? []) as { id: string; email: string | null; full_name: string | null }[];
          ...
          const ranking: RankingEntry[] = rankingRaw.map((r) => {
                      const fallbackName =
                                    r.display_name ||
                                    profileMap[r.user_id]?.full_name ||
                                    emailById[r.user_id]?.split("@")[0] ||
                                    null;
                      return { ...r, display_name: fallbackName };
          });
          ...
          const recipients = authUsers
              .map((u) => ({
                            ...
                            display_name: profileMap[u.id]?.full_name || null,
              }))
