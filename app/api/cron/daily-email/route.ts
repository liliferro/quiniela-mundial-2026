...
try {
        ...
        ? supabase.from("league_rankings").select("user_id, display_name:full_name, total_pts, exact_count:exact_hits, position").eq("league_id", tenantId).order("position").limit(10)
                : supabase.from("league_rankings").select("user_id, display_name:full_name, total_pts, exact_count:exact_hits, position").order("position").limit(10);

        const { data: rankingData, error: rankingError } = await rankQuery;
        if (rankingError) {
        ...
        const payloads = batch.map((user) => {
                ...
                const subject = buildDailyEmailSubject({ todayMatchCount: todayMatches.length, unvotedCount, brandName });
                                   ...
                                   return { from: fromEmail, to: [user.email], subject, html };
        });

        try {
                const { error } = await resend.batch.send(payloads);
                if (error) {
                        console.error(`Error enviando batch ${i}-${i + batch.length}:`, error);
                        failed += batch.length;
                ...
                console.error(`Excepcion enviando batch ${i}-${i + batch.length}:`, err instanceof Error ? err.message : err);
                failed += batch.length;
                failedEmails.push(...batch.map((u) => u.email));
        }
        ...
        return NextResponse.json({ message: "Emails enviados", sent, failed, failedEmails, todayMatches: todayMatches.length, recipients: recipients.length });
} catch (err) {
